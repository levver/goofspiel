...
function App() {
    ...
    const commitPlayerBid = (rank) => {
        ...
        setTimeout(() => resolveRound(rank, cpuChoice), RESOLVE_ROUND_TIMER);
    };
    ...
    const resolveRound = (pBid, cBid) => {
        ...
        setTimeout(() => {
            ...
            // FIX: Use prizeDeck length for definitive game end check.
            // If prizeDeck.length is 0, the last round was just resolved.
            if (prizeDeck.length === 0) {
                setGameState('END');
            } else {
                setGameState('BETTING');
            }
